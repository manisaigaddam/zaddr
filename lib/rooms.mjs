...
function snapshot(room, youMark) {
  ...
  return {
    ...
    score: room.score,
    seriesGoal: room.seriesGoal,
    seriesOver: room.seriesOver,
  };
}
...
function record(room, { winner = null, draw = false, reason = null } = {}) {
  ...
  if (winner && room.score[winner] >= room.seriesGoal) room.seriesOver = true;
}
...
export function attachGame(server) {
  ...
  function onMsg(ws, raw) {
    ...
    if (msg.type === "again") {
      ...
      if (room.seriesOver) resetSeries(room);
      else resetBoard(room);
      broadcast(room);
      maybeCpu(room);
      return;
    }
